...
{
    [HarmonyPatch(typeof(Pawn_HealthTracker), nameof(CheckForStateChange), new[] { typeof(DamageInfo?), typeof(Hediff) }), HarmonyTranspiler]
    public static IEnumerable<CodeInstruction> CheckForStateChange(IEnumerable<CodeInstruction> instructions, ILGenerator gen)
    {
        ...
        Insert(OpCodes.Call, typeof(Mod).GetMethod(nameof(Mod._IsDeathAllowed), AccessTools.all)); // call our method
        ...
        list[Insert(OpCodes.Nop)].WithLabels(lab);
        return list;
    }
}